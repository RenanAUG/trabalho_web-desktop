...
public abstract class GenericDAO<Objeto> {
    ...
    protected abstract Objeto construirObjeto(ResultSet rs) throws SQLException;
    ...
    public Objeto retornarPeloId(int id, String tabela, String chavePrimaria) {
        ...
        try {
            ps = conn.prepareStatement("SELECT * FROM " + tabela + " WHERE " + chavePrimaria + " = ?");
            ps.setInt(1, id);
            rs = ps.executeQuery();
    ...
    public void delete(int id, String tabela, String chavePrimaria) {
        ...
        try {
            ps = conn.prepareStatement("DELETE FROM " + tabela + " WHERE " + chavePrimaria + " = ?");
            ps.setInt(1, id);
            ps.executeUpdate();
            ps.close();
        } catch (SQLException ex) {
